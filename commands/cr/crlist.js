...
module.exports = class CrList extends commando.Command {
	...
	async run(msg, {page, sort, order}) {
		...
		this.client.models.cr.getAll(msg.guild.id).then((result) => {
            ...
            result = result.map((cr, index) => {
                return `${page * 10 + index + 1}. ${cr.trigger.length > 70 ? cr.trigger.substr(0, 70) + '...' : cr.trigger}
    => ${cr.response.length > 70 ? cr.response.substr(0, 70) + '...' : cr.response}`
            ...
            let toSay = `Showing page ${page + 1} of ${totalPages}.\`\`\`\n${result.join('\n')}\`\`\``;
			return msg.say(toSay);
		}).catch((e) => {
			return msg.say('sorry something went wrong: ' +  e);
		});
	}
};
